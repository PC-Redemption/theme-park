...
Mission Control Jinja is the Jinja-first starter implementation in this repo.
...
```bash
python3 -m pip install -r sites/mission-control-jinja/requirements.txt
```

Run:

```bash
uvicorn sites.mission-control-jinja.app:app --reload
```

If your shell does not support importing from a hyphenated path, run:

```bash
cd sites/mission-control-jinja
